...
class Guidance_block(nn.Module):
    """
    Guidance block
    """
    def __init__(self, in_ch, out_ch):
        super(Guidance_block, self).__init__()

        self.conv1 = nn.Conv2d(in_channels=int(in_ch/2), out_channels=out_ch, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(in_channels=int(in_ch/2), out_channels=out_ch, kernel_size=1, stride=1, padding=0)
        self.conv3 = nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=1, stride=1, padding=0)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, v, x_2s):
        b, c, h, w = v.size()
        c = int(c/2)
        v = self.conv3(v)
        recon = F.interpolate(v, scale_factor=2, mode='nearest')

        q = self.conv1(x_2s[:, :, ::2, ::2]).contiguous().view(b, c, -1)
        k = self.conv2(x_2s)
        v = v.contiguous().view(b, c, -1).permute(0, 2, 1)

        k_sample = k[:, :, ::2, 1::2].contiguous().view(b, c, -1).permute(0, 2, 1)
        weight = self.softmax(torch.matmul(k_sample, q))

        recon[:, :, ::2, 1::2] = torch.matmul(weight, v).permute(0, 2, 1).contiguous().view(b, c, h, w)

        k_sample = k[:, :, 1::2, ::2].contiguous().view(b, c, -1).permute(0, 2, 1)
        weight = self.softmax(torch.matmul(k_sample, q))
        recon[:, :, 1::2, ::2] = torch.matmul(weight, v).permute(0, 2, 1).contiguous().view(b, c, h, w)

        k_sample = k[:, :, 1::2, 1::2].contiguous().view(b, c, -1).permute(0, 2, 1)
        weight = self.softmax(torch.matmul(k_sample, q))
        recon[:, :, 1::2, 1::2] = torch.matmul(weight, v).permute(0, 2, 1).contiguous().view(b, c, h, w)

        recon[:, :, ::2, ::2] = v.permute(0, 2, 1).contiguous().view(b, c, h, w)
...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch_rgb=3, in_ch_d=1, out_ch=5):
        ...
        self.Up5 = Guidance_block(filters[4], filters[3])
    ...
    def forward(self, rgb, d):
        ...
        feature = tuple([fuse5, e5_rgb, e5_d])

        d5 = self.Up5(e5_rgb + fuse5, e4_rgb + fuse4)
...
def AttFPIKDAttUpBaseline(in_ch_rgb, in_ch_d, out_ch):
    model = U_Net(in_ch_d=in_ch_d, in_ch_rgb=in_ch_rgb, out_ch=out_ch)
    return model